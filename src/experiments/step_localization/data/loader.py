...
class LMDB_Folder_Dataset(Dataset):
    def __init__(self, folder, split="train", transform=None, truncate=0):
        ...
        self.cls_datasets = [
            LMDB_Class_Dataset(f, split, transform, truncate) for f in cls_folders
        ]
...
class LMDB_Class_Dataset(Dataset):
    def __init__(self, cls_folder, split="train", transform=None, truncate=0):
        ...
        lmdb_filename = (
            osp.basename(cls_folder.rstrip("/")).replace("lmdb", split) + ".lmdb"
        )
    ...
    def __getitem__(self, idx):
        ...
        sample_dict["num_frames"] = np.array(
            full_sample_dict["frames_features"].shape[0]
        )
