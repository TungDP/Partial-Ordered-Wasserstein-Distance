...
def mil_nce(
    features_1, features_2, correspondance_mat, eps=1e-8, gamma=1, hard_ratio=1
):
...
def compute_clust_loss(
    ...
    for i, sample in enumerate(samples):
        ...
        if all_classes_distinct:
            ...
            step_ids = torch.arange(
                global_step_id_count, global_step_id_count + n_samples
            )
        ...
        unique_step_labels, unique_idxs = [
            torch.from_numpy(t)
            for t in np.unique(step_ids.detach().cpu().numpy(), return_index=True)
        ]
    ...
    assert (
        pooled_frames_labels.shape[0] == all_pooled_frames.shape[0]
    ), "Shape mismatch occured"

    unique_labels, unique_idxs = [
        torch.from_numpy(t)
        for t in np.unique(all_step_labels.detach().cpu().numpy(), return_index=True)
    ]
    ...
    xz_label_mat = torch.zeros([N_steps, unique_labels.shape[0]]).to(
        all_pooled_frames.device
    )
...
def compute_alignment_loss(
    ...
    for i, sample in enumerate(samples):
        ...
        zx_costs, drop_costs, _ = compute_all_costs(
            sample, distractor, gamma_xz, drop_cost_type, keep_percentile, l2_normalize
        )
        zx_costs_list.append(zx_costs)
        drop_costs_list.append(drop_costs)

    if dp_algo == "NW":
        min_costs, _ = batch_NW(
            zx_costs_list, drop_costs_list, gamma_min=gamma_min, softning=softning
        )
    else:
