...
def drop_dtw(
    zx_costs, drop_costs, exclusive=True, contiguous=True, return_labels=False
):
    ...
    for zi in range(1, K + 1):
        for xi in range(1, N + 1):
            ...
            if exclusive:
                ...
                neigh_states_pos = (
                    diag_neigh_states + left_pos_neigh_states + top_pos_neigh_states
                )
                neigh_coords_pos = (
                    diag_neigh_coords + left_pos_neigh_coords + top_pos_neigh_coords
                )
                neigh_costs_pos = (
                    diag_neigh_costs + left_pos_neigh_costs + top_pos_neigh_costs
                )
            ...
            P[zi, xi, 1] = (
                *left_neigh_coords[opt_ind_neg],
                left_neigh_states[opt_ind_neg],
            )
...
def double_drop_dtw(
    ...
    D = np.zeros(
        [K + 1, N + 1, 4]
    )  # the 4 dimensions are the following states: zx, z-, -x, --
    ...
    for zi in range(1, K + 1):
        for xi in range(1, N + 1):
            ...
            costs_zx = (
                np.array(neigh_costs_zx) + pairwise_zx_costs[z_cost_ind, x_cost_ind]
            )
...
def lcss(
    zx_costs, drop_costs_x, drop_costs_z=None, exclusive=True, return_labels=False
):
    "LCSS algorithm"
...
def crosstask_dp(cost_matrix, exactly_one=True, bg_cost=0):
    ...
    for t in range(1, T + 1):
        ...
        for k in range(K_ext):
            ...
            j = k
            if (
                (opt_label == -1 or opt_value > Lt[j])
                and Pt[j] != -1
                and (s == 0 or not exactly_one)
            ):
                opt_label = j
                opt_value = Lt[j]
