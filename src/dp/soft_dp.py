...
def batch_dropDTW(
    ...
    for i in range(B):
        ...
        if Ki >= Ni:
            ...
            zx_costs_list[i] = torch.stack([zx_costs_list[i]] * mult, dim=-1).reshape(
                [Ki, -1]
            )
            drop_costs_list[i] = torch.stack(
                [drop_costs_list[i]] * mult, dim=-1
            ).reshape([-1])
            Ni *= mult
        Ns.append(Ni)
        Ks.append(Ki)
    ...
    for zi in range(1, K + 1):
        for xi in range(1, N + 1):
            ...
            if drop_mode == "DropDTW":
                ...
                Dp = (
                    min_fn(pos_neighbors, gamma=gamma_min)
                    + all_zx_costs[z_cost_ind, x_cost_ind]
                )
                ...
                D_final = Dm = Dp = (
                    min_fn([d_diag, d_left, d_right], gamma=gamma_min)
                    + all_zx_costs[z_cost_ind, x_cost_ind]
                )
            D[zi, xi] = torch.cat([D_final, Dm, Dp], dim=0)
...
def batch_NW(zx_costs_list, drop_costs_list, softning="prob", gamma_min=1):
    ...
    for zi in range(1, K + 1):
        for xi in range(1, N + 1):
            ...
            transition_costs = [
                D[zi - 1, xi - 1] + drop_cost,
                D[zi - 1, xi] + match_cost,
                D[zi, xi - 1] + match_cost,
            ]
...
def batch_double_dropDTW(
    zx_costs_list, drop_costs_list, exclusive=True, contiguous=True, gamma_min=1
):
    """function to obtain a soft (differentiable version of DTW)
    embs1, embs2: embedding of size N*D and M*D (N and M : number of video frames
    and D: dimensionality of of the embedding vector)
    ...
    for zi in range(1, K + 1):
        for xi in range(1, N + 1):
            ...
            # DP 3: coming to --
            costs___ = [
                d + all_drop_costs[z_cost_ind] * 2 for d in diag_neigh_costs
            ] + [
